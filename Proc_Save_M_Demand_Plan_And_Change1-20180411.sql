USE [mms]
GO
/****** Object:  StoredProcedure [dbo].[Proc_Save_M_Demand_Plan_And_Change1]    Script Date: 2018-04-11 12:02:15 ******/
SET ANSI_NULLS ON
GO
	...
	begin
		...
		if @Change_State = '' begin
			...
			select @MDDLDID,Drawing_No,PackId,TaskId,DraftId,Technics_Line,ItemCode1,NumCasesSum,Mat_Unit,Quantity,Rough_Size,Dinge_Size,
				...
				'81', @MDPL_Code, @MDPLId, @@IDENTITY, '清单需求', ItemCode1, @tmpNeedsStr, DemandNumSum, NumCasesSum, Rough_Spec,@tmpManufacturerStr,
			...
			select '81', @user_rq_number_start + '-' + convert(nvarchar(50),@MDPLId) + '-' + convert(nvarchar(50),@MDML_Id), @MDPLId, @MDML_Id, '清单需求'
				, ItemCode1, @tmpNeedsStr, DemandNumSum, NumCasesSum, Rough_Spec,@tmpManufacturerStr, @tmpDateStr, @tmpDegreStr
