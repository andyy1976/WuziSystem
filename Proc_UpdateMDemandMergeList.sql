USE [mms]
GO
/****** Object:  StoredProcedure [dbo].[Proc_UpdateMDemandMergeList]    Script Date: 2018-03-28 14:53:37 ******/
SET ANSI_NULLS ON
GO
...
BEGIN transaction T
	...
	set REQUESTER = @REQUESTER,
		...
		Quantity_Left = @NumCasesSum -Quantity_Applied,
		DemandNum_Left = @DemandNumSum -DemandNum_Applied,
		Mat_Unit = @Mat_Unit ,
	...
	if (@Special_Needs != @Special_Needs1 or Convert(decimal(18,4),@NumCasesSum) != Convert(decimal(18,4),@NumCasesSum1) 
		...
		Insert into M_Demand_Plan_List (MDP_Code, PackId, DraftId, [User_ID], Submit_Date,Submit_Type, Submit_State,PlanType)
		values (@MDPL_Code, @PackId, @DraftId, @userId, @curdate, @submit_Type,'0','1')
